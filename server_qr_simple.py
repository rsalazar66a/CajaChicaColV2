...
def extract_qr_from_image_pyzbar(image_path):
    """Extrae códigos QR de imágenes usando solo Pyzbar"""
    try:
...
def extract_qr_from_pdf_simple(pdf_path):
    """Extrae códigos QR de PDF usando pdf2image y Pyzbar"""
    try:
        ...
        for i, pil_image in enumerate(images):
            # Detectar códigos QR en cada página
...
def process_file_real_qr(file_path, file_type):
    ...
    try:
        ...
        if file_type in ['image/jpeg', 'image/jpg', 'image/png', 'image/bmp', 'image/tiff', 'image/tif']:
            qr_data, error = extract_qr_from_image_pyzbar(file_path)
        elif file_type == 'application/pdf':
            qr_data, error = extract_qr_from_pdf_simple(file_path)
        elif file_type in ['image/heic', 'image/heif']:
            qr_data, error = extract_qr_from_heic_simple(file_path)
        else:
    ...
    try:
        ...
        for file in files:
            if file and allowed_file(file.filename):
                ...
                try:
                    # Procesar archivo (REAL con Pyzbar)
...
def health_check():
    return jsonify({
        'status': 'OK', 
        'message': 'Servidor funcionando correctamente - EXTRACCIÓN REAL DE QR CON PYZBAR',
        'libraries': {
            'pyzbar': 'Disponible',
...
if __name__ == '__main__':
    ...
    print("⚠️  NOTA: Usando Pyzbar para decodificación real de QR")
